    ...
    public function verifycode($value = '') {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Auth->login()) {
                ...
                $user['actived'] = 1;
                $this->User->save($user);
                if ($user['role'] == 'lecturer' && $data['Lecturer']['question_verifycode_id'] == $user['Lecturer']['question_verifycode_id'] && $data['Lecturer']['current_verifycode'] == $user['Lecturer']['current_verifycode']) {
