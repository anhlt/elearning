...
class AdminsController extends AppController {
    ...
    public function login() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($user['User']['role'] != 'admin') {
                $this->Session->setFlash(__('ãƒ¦ãƒ¼ã‚¶ã?¯ã?“ã?®ç”»é?¢ã?§ãƒ­ã‚¯ã‚¤ãƒ³ã?§ã??ã?‹ã?„'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-warning'
                ));
            ...
            if ($this->Auth->login()) {
                ...
                if($has_ip == 0){
                    $this->Session->setFlash(__('IPã‚¢ãƒ‰ãƒ¬ã‚¹ã?Œé?•ã?†'), 'alert', array(
                        'plugin' => 'BoostCake',
                        'class' => 'alert-warning'
                    )); 
                    $this->Auth->logout();
                ...
                $this->redirect(array('controller'=>'Admins'));
                    ...
                    $this->Session->setFlash(__('ãƒ¦ãƒ¼ã‚¶å??ã€?ãƒ‘ã‚¹ãƒ¯ãƒ¼ãƒ‰ã?Œæ­£ã?—ã??ã?ªã?„'), 'alert', array(
                        'plugin' => 'BoostCake',
                        'class' => 'alert-warning'
                    ));
        }
    ...
    public function add_ip_address() {
        ...
        if ($this->request->is('post')) {
            ...
            //check empty
            if ($ip_address == NULL) {
                $this->Session->setFlash(__('IPã‚¢ãƒ‰ãƒ¬ã‚¹ã?Œç©ºã?—ã?„'));
                //check exist
            } else if ($this->IpAdmin->query("SELECT * FROM ip_admins WHERE admin_id = '$id' and ip_address = '$ip_address'") != NULL) {
                //$this->Session->setFlash(__('IPã‚¢ãƒ‰ãƒ¬ã‚¹ã?Œå­˜åœ¨ã?—ã?Ÿ'));
                $this->Session->setFlash(__('IPã‚¢ãƒ‰ãƒ¬ã‚¹ã?Œå­˜åœ¨ã?—ã?Ÿ'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-warning'
                ));
                //check format    
                ...
                $this->Session->setFlash(__('IPã‚¢ãƒ‰ãƒ¬ã‚¹ã?®ãƒ•ã‚©ãƒ¼ãƒžãƒƒãƒˆã?Œæ­£ã?—ã??ã?ªã?„'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-warning'
                ));
            }
    ...
    public function edit_ip_address() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->IpAdmin->query("SELECT * FROM ip_admins WHERE admin_id = '$id' and ip_address = '$new_ip_address'") != NULL) {
                ...
                //echo "loi";
                //$this->Session->setFlash(__('IPã‚¢ãƒ‰ãƒ¬ã‚¹ã?®ãƒ•ã‚©ãƒ¼ãƒžãƒƒãƒˆã?Œæ­£ã?—ã??ã?ªã?„'));
                $this->Session->setFlash(__('IPã‚¢ãƒ‰ãƒ¬ã‚¹ã?®ãƒ•ã‚©ãƒ¼ãƒžãƒƒãƒˆã?Œæ­£ã?—ã??ã?ªã?„'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-warning'
                ));
...
//ä»¥ä¸‹ã?¯å…ˆç”Ÿç®¡ç?†ã?®æ©Ÿèƒ½ã? 
        ...
        if ($this->request->is('post')) {
            ...
            if ($username != NULL) {
                ...
                if ($data != NULL) {
                    ...
                    $this->Session->setFlash(__('è¦‹ã?¤ã?‘ã?ªã?„'), 'alert', array(
                        'plugin' => 'BoostCake',
                        'class' => 'alert-warning'
                    ));
                    //$this->redirect(array('action'=>'manage_lecturer'));
    ...
    public function reset_password_lecturer($id_lecturer, $init_password) {
        ...
        if(isset($id_lecturer) && isset($init_password)){
            ...
            $this->User->query($sql);
            //$this->Session->setFlash(__('ãƒ‘ã‚¹ãƒ¯ãƒ¼ãƒ‰ã?®ãƒªã‚»ãƒƒãƒˆã?Œæˆ?åŠŸã?•ã‚Œã?Ÿ'));
            $this->Session->setFlash(__('ãƒ‘ã‚¹ãƒ¯ãƒ¼ãƒ‰ã?®ãƒªã‚»ãƒƒãƒˆã?Œæˆ?åŠŸã?•ã‚Œã?Ÿ'), 'alert', array(
                'plugin' => 'BoostCake',
                'class' => 'alert-success'
            ));
            $this->redirect(array('action' => 'manage_lecturer'));
        }else{
            $this->Session->setFlash(__('ãƒ‘ã‚¹ãƒ¯ãƒ¼ãƒ‰ã?®ãƒªã‚»ãƒƒãƒˆã?Œã?§ã??ã?ªã?„'), 'alert', array(
                'plugin' => 'BoostCake',
                'class' => 'alert-warning'
            ));
            $this->redirect(array('action' => 'manage_lecturer'));
    ...
    public function reset_verifycode_lecturer($id_lecturer, $init_verifycode) {
        ...
        if(isset($id_lecturer) && isset($init_verifycode)){
            ...
            $this->User->query($sql); 
             //$this->Session->setFlash(__('verifycodeã?®ãƒªã‚»ãƒƒãƒˆã?Œæˆ?åŠŸã?•ã‚Œã?Ÿ'));
            $this->Session->setFlash(__('verifycodeã?®ãƒªã‚»ãƒƒãƒˆã?Œæˆ?åŠŸã?•ã‚Œã?Ÿ'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-success'
            ));
            $this->redirect(array('action' => 'manage_lecturer'));
            ...
            $this->Session->setFlash(__('verifycodeã?®ãƒªã‚»ãƒƒãƒˆã?Œã?§ã??ã?ªã?„'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-warning'
                ));
            $this->redirect(array('action' => 'manage_lecturer'));
    ...
    public function delete_lecturer($id_lecturer) {
        ...
        if ($this->User->delete($id_lecturer)) {
            ...
            //$this->Session->setFlash(__('ã‚¢ã‚«ã‚¦ãƒ³ãƒˆã?®å‰Šé™¤ã?Œæˆ?åŠŸã?•ã‚Œã?Ÿ'));        
            $this->Session->setFlash(__('ã‚¢ã‚«ã‚¦ãƒ³ãƒˆã?®å‰Šé™¤ã?Œæˆ?åŠŸã?•ã‚Œã?Ÿ'), 'alert', array(
                'plugin' => 'BoostCake',
                'class' => 'alert-success'
            ));
            //$this->redirect(array('action'=>'manage_lecturer'));
        } else {
            //$this->Session->setFlash(__('ã‚¢ã‚«ã‚¦ãƒ³ãƒˆã?®å‰Šé™¤ã?Œã?§ã??ã?ªã?„'));
            //$notify ="ã‚¢ã‚«ã‚¦ãƒ³ãƒˆã?®å‰Šé™¤ã?Œã?§ã??ã?ªã?„";
            $this->Session->setFlash(__('ã‚¢ã‚«ã‚¦ãƒ³ãƒˆã?®å‰Šé™¤ã?Œã?§ã??ã?ªã?„'), 'alert', array(
                'plugin' => 'BoostCake',
                'class' => 'alert-warning'
            ));
        }
...
//ä»¥ä¸‹ã?¯å­¦ç”Ÿç®¡ç?†ã?®æ©Ÿèƒ½ã? 
    ...
    public function reset_password_student($id_student, $init_password) {
        ...
        if(isset($id_student) && isset($init_password)){
            ...
            //$this->Session->setFlash(__('ãƒ‘ã‚¹ãƒ¯ãƒ¼ãƒ‰ã?®ãƒªã‚»ãƒƒãƒˆã?Œã?§ã??ã?ªã?„'));
            $this->Session->setFlash(__('ãƒ‘ã‚¹ãƒ¯ãƒ¼ãƒ‰ã?®ãƒªã‚»ãƒƒãƒˆã?Œæˆ?åŠŸã?•ã‚Œã?Ÿ'), 'alert', array(
                'plugin' => 'BoostCake',
                'class' => 'alert-warning'
            ));
    ...
    public function reset_verifycode_student($id_student, $init_verifycode) {
        ...
        $sql = "UPDATE students SET current_verifycode = '$init_verifycode' WHERE   students.id = '$id_student'";
            ...
            //$this->Session->setFlash(__('verifycodeã?®ãƒªã‚»ãƒƒãƒˆã?Œã?§ã??ã?ªã?„'));
            $this->Session->setFlash(__('verifycodeã?®ãƒªã‚»ãƒƒãƒˆã?Œã?§ã??ã?ªã?„'), 'alert', array(
                'plugin' => 'BoostCake',
                'class' => 'alert-warning'
            ));
...
//ä»¥ä¸‹ã?¯ã‚·ã‚¹ãƒ†ãƒ ä»•æ§˜ã?®ç®¡ç?†ã?®æ©Ÿèƒ½ã?     
    public function manage_parameter() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($flash) {
                ...
                //$this->Session->setFlash(__('å?„ä»•æ§˜ã?®ã‚¿ã‚¤ãƒ—ã?Œæ•°å­—ã? '));
                $this->Session->setFlash(__('å?„ä»•æ§˜ã?®ã‚¿ã‚¤ãƒ—ã?Œæ•°å­—ã? '), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-warning'
                ));
    ...
    public function add_admin() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->User->saveAll($this->request->data)) {
                ...
                $this->Session->setFlash(__('æ–°ã?—ã?„ç®¡ç?†è€…ã‚’è¿½åŠ ã?§ã??ã?ªã?„'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-warning'
                ));
            }
    ...
    public function remove_admin_process($id) {
        ...
        if ($this->User->delete($id))
            $this->Session->setFlash(__('ç®¡ç?†è€…ã?Œå‰Šé™¤ã?•ã‚Œã?Ÿ'), 'alert', array(
                'plugin' => 'BoostCake',
                'class' => 'alert-success'
            ));
        $this->redirect(array("action" => "remove_admin"));
    ...
    public function fee_manager($year_ = null, $month_ = null) {
        ...
        if ($this->request->is('post')) {
            ...
            if (file_exists($File)) {
                $this->Session->setFlash(__($year . 'å¹´' . $month . 'æœˆã?®TSVã?Œä½œæˆ?ã?•ã‚Œã?¾ã?—ã?Ÿ'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-success'
                ));
            } else {
                if ($this->checkYearOver($year, $month))
                    $this->Session->setFlash(__($year . 'å¹´' . $month . 'æœˆã?®TSVã?Œä½œæˆ?ã?§ã??ã?¾ã?›ã‚“'), 'alert', array(
                        'plugin' => 'BoostCake',
                        'class' => 'alert-warning'
                    ));
            }
    ...
    public function backup_database() {
        ...
        exec($command);
        $this->Session->setFlash(__('ãƒ‡ãƒ¼ã‚¿ãƒ™ãƒ¼ã‚¹ã?Œãƒ?ãƒƒã‚¯ã‚¢ãƒƒãƒ—ã?•ã‚Œã?Ÿ'), 'alert', array(
            'plugin' => 'BoostCake',
            'class' => 'alert-success'));
    ...
    public function delete_all() {
        ...
        $this->Session->setFlash(__('å…¨éƒ¨ã?®ãƒ?ãƒƒã‚¯ã‚¢ãƒƒãƒ—ã?®ãƒ•ã‚¡ã‚¤ãƒ«ã?Œå‰Šé™¤ã?•ã‚Œã?Ÿ'), 'alert', array(
            'plugin' => 'BoostCake',
            'class' => 'alert-warning'));
        $this->redirect(array('controller' => 'admins', 'action' => 'database_manager'));
    }
